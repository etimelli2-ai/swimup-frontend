        ...
        {['tous', 'membre', 'client', 'admin'].map(r => (
          <button key={r} onClick={() => setFiltre(r)}
            className={`flex-1 py-1.5 rounded-full text-xs font-medium transition-all active:scale-95 ${
              filtreRole === r ? 'bg-sky-500 text-white' : 'bg-gray-100 text-gray-600'
            }`}>
            {r === 'tous' ? 'Tous' : r}
          </button>
        ))}
            ...
            <div className="flex bg-gray-100 rounded-xl p-1 gap-1">
              {['infos', 'avis', 'transactions', 'actions'].map(t => (
                <button key={t} onClick={() => setTab(t)}
                  className={`flex-1 py-1.5 text-xs font-medium rounded-lg transition-all ${tab === t ? 'bg-white shadow text-sky-700' : 'text-gray-500'}`}>
                      ...
                      <div className="flex gap-2">
                        {['membre', 'client', 'admin'].map(r => (
                          <button key={r} onClick={() => changeRole(detail.id, r)}
                            ...
                            className={`flex-1 py-2 rounded-full text-xs font-medium flex items-center justify-center gap-1 active:scale-95 transition-all ${
                              detail.role === r ? 'bg-sky-500 text-white' : 'bg-gray-100 text-gray-600'
                            } disabled:opacity-70`}>
